...
// reference from blockmask.tistory.com/344

// Set Console Size and Console Title Name
void SetConsoleView(){
...
void GotoXY(int x, int y){
    COORD pos;  //Cursor Position
    pos.X = 2*x;
    pos.Y = y;
    SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), pos); // 커서 좌표 이동하는 함수인듯
}

// Recieves input from keyboard, and returns value
int GetKeyDown(){
    if (_kbhit() != 0){
        return _getch();
    }
